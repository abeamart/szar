...
module.exports = async (interaction, client, handler) => {
    ...
    if (interaction.customId === 'verkontynuuj') {
		...
		if (verchan == false) { interaction.editReply({ ephemeral: true, content: "hm.. ten kanał został usunięty, kliknij na przycisk 'personalizuj' ponowanie!" })
        ...
        if (!verchan.permissionsFor(interaction.member).has(PermissionsBitField.Flags.ViewChannel)) {
            ...
            interaction.editReply({ ephemeral: true, content: "zrobione!" })
            console.log(`udało się przywrócić ${interaction.member} jego stary kanał personalizacji`)
        } else {
            interaction.editReply({ ephemeral: true, content: "spójrz, czy nie masz dostępnego kanału z personalizacją" })
        ...
        const verchanid = await checkforverchannel(interaction.user.id, interaction.guild)
        const verchan = await canfetch(interaction.guild.channels, verchanid)
		if (verchan == false) { interaction.editReply({ content: "hm.. ten kanał został usunięty, kliknij na przycisk 'personalizuj' ponowanie!" })
            console.log(error)
            interaction.editReply({ content: "okazuje się, że ten kanał został usunięty. stworzę nowy" })
        ...
        if (!verchan.permissionsFor(interaction.member).has(PermissionsBitField.Flags.ViewChannel)) {
            ...
            interaction.editReply({  content: "masz już dostępną sesję personalizacji!" })
